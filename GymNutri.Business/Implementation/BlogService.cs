...
namespace GymNutri.Business.Implementation
{
    public class BlogService : IBlogService
    {
        ...
        public void Update(BlogViewModel blog, out bool result, out string message)
        {
            _blogRepository.Update(Mapper.Map<BlogViewModel, Blog>(blog), out result, out message);
            if (!string.IsNullOrEmpty(blog.Tags))
            {
                ...
                foreach (string t in tags)
                {
                    ...
                    if (!_tagRepository.FindAll(x => x.Id == tagId).Any())
                    {
                        Tag tag = new Tag
                        {
                            Id = tagId,
                            Name = t,
                            Type = CommonConstants.TagType.ProductTag
                        };
                        _tagRepository.Add(tag, out result, out message);
                    }
                    _blogTagRepository.RemoveMultiple(_blogTagRepository.FindAll(x => x.Id == blog.Id).ToList(), out result, out message);
                    BlogTag blogTag = new BlogTag
                    {
                        BlogId = blog.Id,
                        TagId = tagId
                    };
                    _blogTagRepository.Add(blogTag, out result, out message);
                }
            }
        }
        ...
        {
            return _blogRepository.FindAll(x => x.Active == true).OrderByDescending(x => x.DateCreated)
                .Take(top).ProjectTo<BlogViewModel>().ToList();
        }

        public List<BlogViewModel> GetHotProduct(int top)
        {
            return _blogRepository.FindAll(x => x.Active == true && x.HotFlag == true)
        ...
        {
            var query = _blogRepository.FindAll(x => x.Active == true);
        ...
        {
            return _blogRepository.FindAll(x => x.Active == true
        ...
        {
            var query = _blogRepository.FindAll(x => x.Active == true && x.Name.Contains(keyword));
        ...
        {
            return _blogRepository.FindAll(x => x.Active == true && x.Id != id)
            .OrderByDescending(x => x.DateCreated)
            .Take(top)
            .ProjectTo<BlogViewModel>()
            .ToList();
        ...
        public List<BlogViewModel> GetListByTag(string tagId, int page, int pageSize, out int totalRow)
        {
            var query = from p in _blogRepository.FindAll()
                        ...
                        where pt.TagId == tagId && p.Active == true
        ...
        {
            return _tagRepository.FindAll(x => x.Type == CommonConstants.TagType.ProductTag
